...
(cor_coeff <- cor(joined_count_by_cat %>% select(contains("AL-hurricane")) %>% unlist(),
  joined_count_by_cat %>% select(contains("PC-hurricane")) %>% unlist(), method = "pearson")
)
```
```{r error=TRUE}
...
(count_plot <- joined_count_by_cat %>% 
  ...
  geom_jitter() +
  labs(x = 'Number of hurricanes in the Atlantic', y = 'Number of hurricanes in the Pacific', 
...
(rough_fit <- fit(rough_spec, data = joined_count_by_cat, 
...
(rough_info <- summary(rough_fit_engine))
```
```{r error=TRUE}
...
quad_fit <- fit(quad_spec, data = joined_count_by_cat, formula = `AL-hurricane` ~ poly(`PC-hurricane`, 2))

quad_aug <- broom::augment(quad_fit, new_data = joined_count_by_cat)

quad_fit_engine <- extract_fit_engine(quad_fit)

# Obtain summary of the fitted model
quad_info <- summary(quad_fit_engine)
```
```{r error=TRUE}
...
download_xlsx <- function(remote_xlsx_file = sea_ice[[1]], local_xlsx_file = sea_ice_files[[1]]){
  download.file(url = remote_xlsx_file, destfile = local_xlsx_file, method = "auto", mode = "wb")
}
...
ice_extent_file <- function(remote_xlsx_file = sea_ice[[1]], local_xlsx_file = sea_ice_files[[1]]){
  ...
  data_sheets <- sheet_names[sapply(sheet_names, function(sheet) grepl("Area|Extent", sheet, ignore.case = TRUE))]  
  purrr::map(
    data_sheets,
...
```{r error=TRUE, tags=c()}
# Testing the function with the Northern hemisphere file
ice_extent_file()
```
...
(log_seaice <- seaice %>%
  mutate(date= make_date(year=seaice$year,month=seaice$month, day = seaice$day),
         log_area = log2(ifelse(Area > 0, Area, NA)),
         log_extent = log2(ifelse(Extent > 0, Extent, NA))) %>%
  filter(!is.na(log_area) & !is.na(log_extent) & !is.na(date)) )
...
# Create the split variable using a valid SplitRatio
